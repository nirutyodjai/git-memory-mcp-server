...
const SplineWrapper = react_1.default.forwardRef(({ scene, onLoad, onError, style, className, fallback }, ref) => {
    ...
    if (hasError) {
        return (<div className={`flex items-center justify-center bg-gray-100 dark:bg-gray-800 ${className || ''}`} style={style}>
    ...
    return (<react_1.Suspense fallback={<div className={`flex items-center justify-center bg-gray-50 dark:bg-gray-900 ${className || ''}`} style={style}>
        <div className="text-center p-8">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mx-auto mb-4"></div>
          <p className="text-sm text-gray-600 dark:text-gray-300">Loading 3D model...</p>
        </div>
      </div>}>
      <Spline ref={ref} scene={scene} onLoad={handleLoad} onError={handleError} style={style} className={className}/>
    </react_1.Suspense>);
});
SplineWrapper.displayName = 'SplineWrapper';
exports.default = SplineWrapper;
//# sourceMappingURL=spline-wrapper.js.map