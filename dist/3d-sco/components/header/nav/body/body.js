...
function Body({ links, selectedLink, setSelectedLink, setIsActive, }) {
    ...
    const getChars = (word) => {
        ...
        word.split("").forEach((char, i) => {
            chars.push(React.createElement(framer_motion_1.motion.span, { className: "pointer-events-none", custom: [i * 0.02, (word.length - i) * 0.01], variants: anim_1.translate, initial: "initial", animate: "enter", exit: "exit", key: char + i }, char));
        });
        return chars;
    };
    return (React.createElement("div", { className: (0, utils_1.cn)(style_module_scss_1.default.body, "flex flex-col items-end md:flex-row") }, links.map((link, index) => {
        const { title, href, target } = link;
        return (React.createElement(link_1.default, { key: `l_${index}`, href: href, target: target, className: "cursor-can-hover rounded-lg" },
            React.createElement(framer_motion_1.motion.p, { className: (0, utils_1.cn)("rounded-lg", currentHref !== href ? "text-muted-foreground" : "underline"), onClick: () => setIsActive(false), onMouseOver: () => setSelectedLink({ isActive: true, index }), onMouseLeave: () => setSelectedLink({ isActive: false, index }), variants: anim_1.blur, animate: selectedLink.isActive && selectedLink.index !== index
                    ? "open"
                    : "closed" }, getChars(t(title, 'navigation')))));
    })));
}
