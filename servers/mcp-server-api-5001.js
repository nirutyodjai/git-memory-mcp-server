...
class mcpserverapi5001Server {
    constructor() {
        this.name = 'mcp-server-api-5001';
        this.port = 5001;
        this.category = 'api';
    ...
    setupHandlers() {
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                {
                    name: 'api_operation',
                    description: 'Perform api operations for mcp-server-api-5001',
                    inputSchema: {
                        type: 'object',
                        properties: {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            if (name === 'api_operation') {
                return {
                    content: [{
                        type: 'text',
                        text: `API operation '${args.action}' completed successfully on ${this.name}`
                    }]
                };
            }
...
const server = new mcpserverapi5001Server();
