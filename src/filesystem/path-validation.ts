...
export function isPathWithinAllowedDirectories(absolutePath: string, allowedDirectories: string[]): boolean {
  ...
  return allowedDirectories.some(dir => {
    ...
    if (normalizedDir === path.sep) {
      // On Windows, Unix-style root '/' should not match Windows paths with drive letters
      if (path.sep === '\\' && normalizedPath.match(/^[A-Za-z]:\\\\/)) {
        return false;
      }
      // On Unix-like systems, check if path starts with root separator
      if (path.sep === '/') {
        return normalizedPath.startsWith(path.sep);
      }
      // On Windows, Unix root '/' should not match any Windows paths
      return false;
    }
    
    // On Windows, check if the original allowed directory was Unix root '/'
    // but got normalized to current drive root (e.g., 'D:\\')
    if (path.sep === '\\' && dir === '/' && normalizedDir.match(/^[A-Za-z]:\\?$/)) {
      // Unix root '/' should not match Windows paths with different drive letters
      const dirDrive = normalizedDir.charAt(0).toLowerCase();
      const pathDrive = normalizedPath.charAt(0).toLowerCase();
      if (pathDrive !== dirDrive) {
        return false;
      }
    }
    ...
    if (path.sep === '\\' && normalizedDir.match(/^[A-Za-z]:\\?$/)) {
      // If the original directory was Unix root '/', only allow Unix-style paths
      if (dir === '/') {
        // Don't allow Windows paths with drive letters when allowed dir is Unix root '/'
    ...
    // Final check: ensure path starts with the allowed directory
    // On Windows, don't allow Unix root '/' to match Windows paths
    if (path.sep === '\\' && normalizedDir === '/' && normalizedPath.match(/^[A-Za-z]:\\\\/)) {
      return false;
    }
