...
import { useHealthCheck, useUsers, usePosts, useComments, useAnalytics, useTrackAnalytics } from '@/hooks/useApi'
...
const DatabaseDashboard: React.FC = () => {
  const [stats, setStats] = useState<DatabaseStats>({ users: 0, posts: 0, comments: 0, analytics: 0 })
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'posts' | 'comments' | 'analytics'>('overview')
  
  const { data: healthData, loading: healthLoading, error: healthError } = useHealthCheck()
  const { data: usersData, loading: usersLoading, refetch: refetchUsers } = useUsers({ limit: 5 })
  const { data: postsData, loading: postsLoading, refetch: refetchPosts } = usePosts({ limit: 5 })
  const { data: commentsData, loading: commentsLoading, refetch: refetchComments } = useComments({ limit: 5 })
        ...
        {activeTab === 'users' && (
          <div>
            ...
            {usersLoading ? (
              ...
              <p className="text-gray-500">ไม่มีข้อมูลผู้ใช้งาน</p>
            )}
          </div>
        )}
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-2">
                        {comment.author.avatar ? (
                          <img src={comment.author.avatar} alt={comment.author.username} className="w-8 h-8 rounded-full" />
                        ) : (
        ...
        {activeTab === 'analytics' && (
          <div>
            ...
            {analyticsLoading ? (
              ...
              <p className="text-gray-500">ไม่มีข้อมูล Analytics</p>
            )}
          </div>
        )}
      </div>
