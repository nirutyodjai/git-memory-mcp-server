...
import { webFetcher } from './mcp-tools';

// Blender Scene Information Types
export interface BlenderSceneInfo {
  ...
  collections: string[];
  frameRange: { start: number; end: number; current: number };
  renderSettings: {
    engine: string;
...
export class BlenderTools {
  private static instance: BlenderTools;
  private blenderConnected = false;
  private blenderPort = 8080;

  private constructor() {}

  public static getInstance(): BlenderTools {
  ...
  async checkBlenderConnection(): Promise<boolean> {
    try {
  ...
  async executeBlenderCode(code: string): Promise<{
    success: boolean;
    result?: any;
    error?: string;
  }> {
  ...
  async getSceneInfo(): Promise<BlenderSceneInfo | null> {
    const code = `
import bpy
import json
...
`;
  ...
  async getObjectInfo(objectName: string): Promise<BlenderObject | null> {
    const code = `
import bpy
import json
...
`;
  ...
  async getViewportScreenshot(maxSize: number = 800): Promise<string | null> {
    const code = `
import bpy
import os
import tempfile
...
export class PolyhavnTools {
  private static instance: PolyhavnTools;
  private baseUrl = 'https://api.polyhaven.com';

  private constructor() {}

  public static getInstance(): PolyhavnTools {
  ...
  async getCategories(assetType: 'hdris' | 'textures' | 'models' | 'all' = 'hdris'): Promise<string[]> {
    try {
      const response = await webFetcher.fetchJSON(`${this.baseUrl}/categories/${assetType}`);
  ...
  async searchAssets(
    assetType: 'hdris' | 'textures' | 'models' | 'all' = 'all',
    categories?: string
  ): Promise<PolyhavnAsset[]> {
    try {
      ...
      const response = await webFetcher.fetchJSON(url);
  ...
  async getAssetDownloadUrl(
    assetId: string,
    assetType: 'hdris' | 'textures' | 'models',
    resolution: string = '1k',
    format?: string
  ): Promise<string | null> {
    try {
      const response = await webFetcher.fetchJSON(`${this.baseUrl}/files/${assetId}`);
...
export const blenderTools = BlenderTools.getInstance();
export const polyhavnTools = PolyhavnTools.getInstance();