...
import { Committer, CommitResult, CommitterConfig } from './Committer';
...
export class AutoFixPipeline extends EventEmitter {
  ...
  private async executePipelineStages(run: PipelineRun): Promise<void> {
    ...
    run.verificationResult = await this.verifier.verifyFix(run.fix, issue);
    run.metadata.performance.verificationTime = Date.now() - verifyStartTime;
    
    if (!run.verificationResult.success) {
      throw new Error(`Fix verification failed: ${run.verificationResult.message}`);
    }
  ...
  private requiresApproval(run: PipelineRun): boolean {
    ...
    if (autoApprove.testsPassing && verificationResult && verificationResult.success) {
      const allTestsPassed = verificationResult.checks.every(check => 
        check.status === 'passed'
      );
      if (allTestsPassed) {
        return false;
