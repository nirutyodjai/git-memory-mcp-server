...
declare global {
  ...
  var performanceCounters: any;
  var performanceStart: any;
  var performanceEnd: any;
}
...
async function cleanupTestDirectories() {
  ...
  for (const dir of directories) {
    try {
      ...
      console.warn(`⚠️ Failed to cleanup directory ${dir}:`, (error as Error).message);
    }
  }
...
async function cleanupTestGitRepository() {
  ...
  try {
    ...
    console.warn('⚠️ Test Git repository cleanup failed:', (error as Error).message);
  }
}
...
function cleanupMockServices() {
  if (global.mockServices) {
    ...
    Object.values(global.mockServices).forEach((service: any) => {
      Object.values(service).forEach((mockFn: any) => {
