...
const EnhancedMonitoring = require('./enhanced-monitoring');
...
class APIGatewayMain {
    ...
    async loadConfiguration() {
        try {
            ...
            this.configManager = new APIGatewayConfig(this.configPath);
            await this.configManager.loadConfiguration();
            this.config = this.configManager.getConfig();
            
            // Load monitoring configuration
            try {
                const monitoringConfigData = await fs.readFile(this.monitoringConfigPath, 'utf8');
                this.monitoringConfig = JSON.parse(monitoringConfigData);
                console.log('✅ Monitoring configuration loaded');
            } catch (error) {
                console.warn('⚠️ Monitoring config not found, using defaults');
