...
export class AIContextAwarenessSystem extends EventEmitter {
  ...
  private async loadExistingContexts(): Promise<void> {
    try {
      if (this.gitMemoryService) {
        ...
        const storedContexts = await this.gitMemoryService.retrieve('ai_contexts');
        if (storedContexts) {
  ...
  public searchContexts(criteria: {
    ...
    if (criteria.source) {
      results = results.filter(ctx => ctx.source.includes(criteria.source));
    }
  ...
  private async generateInsights(context: AIContext): Promise<ContextualInsight[]> {
    ...
    try {
      ...
      if (this.semanticMemoryService) {
        const semanticInsights = await this.semanticMemoryService.search(
          JSON.stringify(context.data), { limit: 5 }
        );
  ...
  private async persistContext(context: AIContext): Promise<void> {
    if (this.gitMemoryService) {
      await this.gitMemoryService.store(
        `ai_contexts`,
        { data: JSON.stringify(this.contextHistory) }
      );
    }
  }
