...
/**
 * Function Notebook - Manages function patterns with Git-based memory
 */
export class FunctionNotebook extends EventEmitter {
  ...
  /**
   * Initialize the notebook by loading existing patterns
   */
  ...
  addPattern(pattern: FunctionPattern): void {
    if (this.patterns.length >= this.config.maxPatterns) {
      throw new Error(`Maximum patterns limit (${this.config.maxPatterns}) reached.`);
    }
   ...
   */
  private async savePattern(pattern: FunctionPattern): Promise<void> {
    try {
      const filePath = path.join(this.config.storagePath, `${pattern.id}.json`);
      await fs.writeFile(filePath, JSON.stringify(pattern, null, 2));
      this.logger.info(`Pattern ${pattern.id} saved.`);
    } catch (error) {
      this.logger.error(`Failed to save pattern ${pattern.id}:`, error);
    }
  }
