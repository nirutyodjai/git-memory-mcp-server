...
class APIGatewayConfig extends EventEmitter {
        ...
        return Joi.object({
            ...
            monitoring: Joi.object({
                ...
                dashboard: Joi.object({
                    enabled: Joi.boolean().default(true),
                    path: Joi.string().default('/dashboard')
                }).default(),
                logging: Joi.object({
    ...
    async createDefaultConfig() {
        const minimalValidConfig = {
            auth: {
                jwtSecret: 'your-default-secret-key-please-change'
            }
        };

        // Generate default configuration from schema, using a minimal valid config
        const { value: defaultConfig, error } = this.schema.validate(minimalValidConfig);
        
        if (error) {
            console.error('❌ Failed to generate default configuration from schema:', error.message);
            throw error;
        }
        ...
        console.log('📄 Created complete default configuration file from schema.');
    }
