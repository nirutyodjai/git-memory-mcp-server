...
class SystemValidator {
    ...
    async validateSecurityServers() {
        const { securityStart, securityEnd, batchSize } = this.config;
        const totalServers = securityEnd - securityStart + 1;
        
        console.log(`Testing ports ${securityStart}-${securityEnd}...`);
        console.log('');
        
        const batches = this.createBatches(securityStart, securityEnd, batchSize);
        let totalHealthy = 0;
        let totalFailed = 0;
        ...
        console.log('');
        console.log('🔒 SECURITY SERVERS VALIDATION COMPLETE');
        console.log('================================================================================');
        console.log(`📊 Total Security Servers: ${totalServers}`);
        ...
        // Overall status
        if (totalFailed === 0) {
            console.log('🎉 ALL SECURITY SERVERS OPERATIONAL!');
        } else if (totalHealthy >= totalServers * 0.8) {
            console.log('⚠️  SYSTEM WARNING: Some security servers are down');
            console.log(`💡 Recommendation: Restart ${totalFailed} failed security servers`);
        } else {
            console.log('🚨 SYSTEM CRITICAL: Many security servers are down');
            console.log(`💡 Recommendation: Immediate restart of ${totalFailed} failed security servers`);
        }
    ...
    async run() {
        try {
            ...
            const results = await this.validateSecurityServers();
