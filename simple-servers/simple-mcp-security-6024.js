...
class simplemcpsecurity6024SimpleServer {
    constructor() {
        this.name = 'simple-mcp-security-6024';
        this.port = 6024;
    ...
    handleCallTool(req, res) {
        ...
        req.on('end', () => {
            try {
                ...
                if (tool === 'security_operation') {
                    const result = {
                        success: true,
                        result: `SECURITY operation '${args?.action || 'default'}' completed successfully on ${this.name}`,
                        server: this.name,
                        category: this.category,
                        timestamp: new Date().toISOString()
                    };
...
const server = new simplemcpsecurity6024SimpleServer();
