...
class simplemcpweb6020SimpleServer {
    constructor() {
        this.name = 'simple-mcp-web-6020';
        this.port = 6020;
        this.category = 'web';
    ...
    handleCallTool(req, res) {
        ...
        req.on('end', () => {
            try {
                ...
                if (tool === 'web_operation') {
                    const result = {
                        success: true,
                        result: `WEB operation '${args?.action || 'default'}' completed successfully on ${this.name}`,
                        server: this.name,
                        category: this.category,
                        timestamp: new Date().toISOString()
                    };
...
const server = new simplemcpweb6020SimpleServer();
