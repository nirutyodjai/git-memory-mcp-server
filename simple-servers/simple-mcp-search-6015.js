...
class simplemcpsearch6015SimpleServer {
    constructor() {
        this.name = 'simple-mcp-search-6015';
        this.port = 6015;
    ...
    handleCallTool(req, res) {
        ...
        req.on('end', () => {
            try {
                ...
                if (tool === 'search_operation') {
                    const result = {
                        success: true,
                        result: `SEARCH operation '${args?.action || 'default'}' completed successfully on ${this.name}`,
                        server: this.name,
                        category: this.category,
                        timestamp: new Date().toISOString()
                    };
...
const server = new simplemcpsearch6015SimpleServer();
