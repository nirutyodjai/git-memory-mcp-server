...
class simplemcpqueue6049SimpleServer {
    constructor() {
        this.name = 'simple-mcp-queue-6049';
        this.port = 6049;
        this.category = 'queue';
    ...
    handleCallTool(req, res) {
        ...
        req.on('end', () => {
            try {
                ...
                if (tool === 'queue_operation') {
                    const result = {
                        success: true,
                        result: `QUEUE operation '${args?.action || 'default'}' completed successfully on ${this.name}`,
                        server: this.name,
                        category: this.category,
                        timestamp: new Date().toISOString()
                    };
...
const server = new simplemcpqueue6049SimpleServer();
