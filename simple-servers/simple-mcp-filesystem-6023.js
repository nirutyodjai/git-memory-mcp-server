...
class simplemcpfilesystem6023SimpleServer {
    constructor() {
        this.name = 'simple-mcp-filesystem-6023';
        this.port = 6023;
    ...
    handleCallTool(req, res) {
        ...
        req.on('end', () => {
            try {
                ...
                if (tool === 'filesystem_operation') {
                    const result = {
                        success: true,
                        result: `FILESYSTEM operation '${args?.action || 'default'}' completed successfully on ${this.name}`,
                        server: this.name,
                        category: this.category,
                        timestamp: new Date().toISOString()
                    };
...
const server = new simplemcpfilesystem6023SimpleServer();
