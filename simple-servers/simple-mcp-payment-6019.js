...
class simplemcppayment6019SimpleServer {
    constructor() {
        this.name = 'simple-mcp-payment-6019';
        this.port = 6019;
    ...
    handleCallTool(req, res) {
        ...
        req.on('end', () => {
            try {
                ...
                if (tool === 'payment_operation') {
                    const result = {
                        success: true,
                        result: `PAYMENT operation '${args?.action || 'default'}' completed successfully on ${this.name}`,
                        server: this.name,
                        category: this.category,
                        timestamp: new Date().toISOString()
                    };
...
const server = new simplemcppayment6019SimpleServer();
