...
class simplemcpmedia6036SimpleServer {
    constructor() {
        this.name = 'simple-mcp-media-6036';
        this.port = 6036;
        this.category = 'media';
    ...
    handleCallTool(req, res) {
        ...
        req.on('end', () => {
            try {
                ...
                if (tool === 'media_operation') {
                    const result = {
                        success: true,
                        result: `MEDIA operation '${args?.action || 'default'}' completed successfully on ${this.name}`,
                        server: this.name,
                        category: this.category,
                        timestamp: new Date().toISOString()
                    };
...
const server = new simplemcpmedia6036SimpleServer();
