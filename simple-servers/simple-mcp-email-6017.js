...
class simplemcpemail6017SimpleServer {
    constructor() {
        this.name = 'simple-mcp-email-6017';
        this.port = 6017;
        this.category = 'email';
    ...
    handleCallTool(req, res) {
        ...
        req.on('end', () => {
            try {
                ...
                if (tool === 'email_operation') {
                    const result = {
                        success: true,
                        result: `EMAIL operation '${args?.action || 'default'}' completed successfully on ${this.name}`,
                        server: this.name,
                        category: this.category,
                        timestamp: new Date().toISOString()
                    };
...
const server = new simplemcpemail6017SimpleServer();
