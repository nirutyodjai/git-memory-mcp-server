...
class simplemcpnotification6030SimpleServer {
    constructor() {
        this.name = 'simple-mcp-notification-6030';
        this.port = 6030;
    ...
    handleCallTool(req, res) {
        ...
        req.on('end', () => {
            try {
                ...
                if (tool === 'notification_operation') {
                    const result = {
                        success: true,
                        result: `NOTIFICATION operation '${args?.action || 'default'}' completed successfully on ${this.name}`,
                        server: this.name,
                        category: this.category,
                        timestamp: new Date().toISOString()
                    };
...
const server = new simplemcpnotification6030SimpleServer();
