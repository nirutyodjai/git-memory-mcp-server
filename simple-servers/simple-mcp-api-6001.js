...
class simplemcpapi6001SimpleServer {
    constructor() {
        this.name = 'simple-mcp-api-6001';
        this.port = 6001;
        this.category = 'api';
    ...
    handleCallTool(req, res) {
        ...
        req.on('end', () => {
            try {
                ...
                if (tool === 'api_operation') {
                    const result = {
                        success: true,
                        result: `API operation '${args?.action || 'default'}' completed successfully on ${this.name}`,
                        server: this.name,
                        category: this.category,
                        timestamp: new Date().toISOString()
                    };
...
const server = new simplemcpapi6001SimpleServer();
