...
class simplemcpchat6018SimpleServer {
    constructor() {
        this.name = 'simple-mcp-chat-6018';
        this.port = 6018;
        this.category = 'chat';
    ...
    handleCallTool(req, res) {
        ...
        req.on('end', () => {
            try {
                ...
                if (tool === 'chat_operation') {
                    const result = {
                        success: true,
                        result: `CHAT operation '${args?.action || 'default'}' completed successfully on ${this.name}`,
                        server: this.name,
                        category: this.category,
                        timestamp: new Date().toISOString()
                    };
...
const server = new simplemcpchat6018SimpleServer();
